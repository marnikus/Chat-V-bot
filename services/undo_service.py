...
class UndoService:
    ...
    def _apply_db_command(self, value: dict, forward: bool) -> bool:
        ...
        if str(value.get("op") or "") == "delete":
            work = self._db_delete_round(value, forward)
        else:
            work = self._db_switch_round(value, forward)
        self._timeline_commit.spawn("db command", work)
        return True

    async def _db_delete_round(self, value: dict, forward: bool) -> None:
        """Re-delete the file (forward) or restore its backup (undo)."""
        path = str(value.get("path") or "")
        backup = str(value.get("backup") or "")
        manager = self._dbs
        result = None
        if forward and os.path.exists(path):
        ...
        if result.get("ok"):
            await restart_world(self._memory, self._archive,
                                self._labels, self, self._bus, "delete")
        emit_db_change(self._bus, "delete", result)

    async def _db_switch_round(self, value: dict, forward: bool) -> None:
        """Re-apply a create/load/clean (forward) or its reverse (undo)."""
        op = str(value.get("op") or "")
        result = await self._switch_step(self._dbs, op, forward, value)
        if result is None:
            return
    ...
    async def _switch_step(self, manager, op: str, forward: bool,
                           value: dict):
        """The one DB call a switch command stands for; None = not ours."""
        path = str(value.get("path") or "")
        if forward:
