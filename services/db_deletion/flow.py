"""Fail-closed permanent-deletion pipeline (the mutating phases).
...
scans, footprint and plan) lives in ``services.db_deletion.scan``; this
module owns the mutating phases:

    validate → scan → switch → detach
...
Patch seam: the revalidation calls ``db_deletion.build_deletion_inventory``
through the package, so `test_cancel_concurrency`'s patch lands.
"""
...
from .constants import DB_GROUP_SUFFIXES
from .media import scan_world_media
from .outcome import DeletionOutcome
from .paths import abspath_or_none, lexists
...
async def delete_world(lifecycle, path: str) -> dict:
    ...
    # (scan imports the shared helpers above at module load).
