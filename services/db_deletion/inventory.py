"""World inventory for the safety scan — what exists before we delete.

Enumerates every supported world (active folder + active file +
victim-directory `*.db` scan + remembered in-root `.db` paths) and dedupes by
canonical path. Any source failure marks the inventory incomplete, so the
caller refuses rather than treating an under-count as "no other worlds".

Imports `paths` (canonical); no imports from the mutating half.
"""
...
class _InventoryCollector:
    ...
    def build(self) -> DeletionInventory:
        ...
        victim_c = db_deletion.canonical(self.victim_abs) if self.victim_abs else ""
        in_scope = any(
            db_deletion.canonical(p) == victim_c for p in deduped) if victim_c else False
        worlds = [p for p in deduped if db_deletion.canonical(p) != victim_c]
