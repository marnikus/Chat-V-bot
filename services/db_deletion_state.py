"""Shared primitives of the deletion pipeline: state, refusal, path tests.

Split out of `services.db_deletion_flow` in round H (H2) so the pre- and
post-boundary phase modules can both import them without importing each
other. Nothing here performs I/O beyond `os.path` tests, and nothing here
knows the order of the phases -- that lives in `db_deletion_flow`.

`_PhaseRefusal` is the pipeline's control-flow signal. It subclasses
Exception, which makes one rule non-negotiable: the `raise` must never sit
inside a broad `except Exception`, because an enclosing catch-all disarms it
and turns a safety guard into dead code. Round G found exactly that bug in
this pipeline; the separation here makes the raise sites easy to audit.
"""
