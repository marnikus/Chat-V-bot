...
from .collector.state import CollectorState, IDLE_STATES, DEFAULTS, MAX_PROBE_PENALTY
from .collector.probe import CollectorProbe
from .collector.archive import CollectorArchive
...
class Collector(QObject):
    ...
    async def _tick(self) -> str:
        """Advance one heartbeat through the explicit private-chat phases."""
        state = await CollectorProbe.read(self, chat_agent_js.AGENT_VERSION)
        refused = CollectorProbe.classify(self, state)
        if refused is not None:
            return refused
        refused = CollectorProbe.identify(self, state)
        if refused is not None:
            return refused
        nick = " ".join(str(state.get("partner") or "").split()).strip()
        person_id, head_sig, tail_sig = await CollectorArchive.prepare(
            self, state, nick, _signature)
        refused = CollectorProbe.verify(self, state, nick, verify_private)
        if refused is not None:
            return refused
        return await CollectorArchive.advance(
            self, state, nick, self.my_nick, person_id, head_sig, tail_sig)
