...
async def deliver(cdp, nick: str, text: str, parser=None) -> Result[str]:
    ...
    if allowed.is_err:
        return allowed
    # Round H step H5 split backend/message_injector.py in two: typing stayed,
    # and the send-button half (click_send and its icon fallback) moved to
    # backend/send_button.py. Import each from the module that OWNS it rather
    # than leaning on a re-export, which is the convention the rest of the
    # tree follows after that round.
