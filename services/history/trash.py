...
async def _marked_counts(host, marks: str, params: tuple) -> tuple:
    """The persons/messages the dropped tokens still keep alive."""
...
async def purge_tokens(host, tokens: list) -> dict:
    ...
    params = tuple(clean)
    people, messages = await _marked_counts(host, marks, params)
