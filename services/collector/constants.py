"""The passive collector's vocabulary and defaults.

`CollectorState` is the status vocabulary the feature request asked for;
`IDLE_STATES` picks the slow heartbeat for the states where nothing is
happening; `DEFAULTS` is the settings surface `configure` validates against.
"""
