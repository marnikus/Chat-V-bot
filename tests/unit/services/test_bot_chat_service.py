...
class TestDeliver(unittest.TestCase):
    ...
    def test_a_failed_type_stops_before_clicking_send(self):
        # H5 split the send-button half out of message_injector, so the two
        # halves of this two-step now live in two modules; patch each where
        # it is DEFINED, since bot_chat imports them by module at call time.
        from backend import message_injector, send_button
        calls = []
        ...
        original = (message_injector.type_message, send_button.click_send)
        message_injector.type_message, send_button.click_send = (no_type,
                                                                 click)
        try:
            result = self.deliver("hi")
        finally:
            message_injector.type_message, send_button.click_send = original
    ...
    def test_a_delivered_message_answers_with_its_text(self):
        ...
        original = (message_injector.type_message, send_button.click_send)
        message_injector.type_message, send_button.click_send = (ok, ok)
        try:
            result = self.deliver("hi")
        finally:
            message_injector.type_message, send_button.click_send = original
...
class TestTheRecipientIsVerified(unittest.TestCase):
    ...
    def setUp(self):
        ...
        send_button.click_send = clicked
        self.injector = message_injector
        self.sender = send_button

    def tearDown(self):
        (self.injector.type_message,
         self.sender.click_send) = self._original
