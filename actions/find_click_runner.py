"""Shared two-phase "find → visually confirm → click" routine.

Used by every search-and-click block (CUSTOM_FIND, CLICK_MAIN_TAB, CLICK_BACK)
so they all provide the same visual confirmation and the same log contract:
    ...
    *,
    ...
    try:
        ...
        _report(engine, f"❌ FIND failed: CDP error during element search: {exc}",
                "error")
        log.error("find_and_click CDP error (find): %s", exc)
        return ActionResult.FAIL
