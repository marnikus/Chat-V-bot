...
class AttachImage(BaseAction):
    ...
    async def execute(self, user_nick: str, cdp: CDPClient) -> str:
        ...
        log.info("🖼️ Attaching image: folder='%s', pattern='%s', mode=%s",
                 self.folder_path, self.file_pattern, self.rotation_mode)
        ok = await attach_image(cdp, self.folder_path,
                                self.file_pattern, self.rotation_mode)
        if ok:
            log.info("✅ Image attached successfully")
            return ActionResult.OK
        log.error("❌ FAIL: image attachment failed (check folder and pattern)")
