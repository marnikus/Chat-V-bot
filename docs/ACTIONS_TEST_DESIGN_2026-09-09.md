...
| BUG-02 | **P0** | **FIXED** (`docs/BLOCKERS_FIX_DESIGN_2026-09-09.md` §2) | `backend/history_db.py` + cluster (blocked `actions/collect_history.py`) | The §12 assumption was wrong: the canonical schema was never lost — `stores/history_db.py` holds it (the regroup copied `backend/ → stores/` then broke the `backend/` originals). Fix: `backend/history_db|history_repo|history_models|media_store|label_store|user_memory.py` are now re-export shims over `stores/`; the dead `history_db_parts/` + `history_repo_parts/` dirs are deleted. REG-02/CH-00 `expectedFailure`s dropped (assert normally); the contract file's stub dance is now a per-test recorder patch. The same sweep also restored the split-dropped `collector_parts` names (`DEFAULTS`, `log`, `CollectorState`, `IDLE_STATES`, `MAX_PROBE_PENALTY`, 4 Qt signals, `sync_conversation`/`UserRecord`/`verify_private`/`HistoryQuery`, chat_parser `AppendResult`). | before: `pytest tests/test_db_schema_migration.py` → collection `NameError`; after: 28 passed; full suite 837→1492 passed, errors 52→10 (rest are N1–N4/filed, Router/events/services seams) |
