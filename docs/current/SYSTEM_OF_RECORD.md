...
| Wire | `bridge/` (17) | `bridge/router.py` — ONE QObject on the QWebChannel, assembled from thirteen domain bridges: cdp · stack · people · history · label · db · collector · undo · layout · file · window-preset · bot · bot-prompt · bot-settings |
| Orchestration | `services/` (67) | `run/` (engine), `history/` (service + `trash.py` session-sized trash + `migrate.py` install migration), collector (the `collector_*` family), db lifecycle + deletion (the `db_deletion_*` family), layout, people, undo (the `undo_*` family: `undo_service.py` facade + `undo_history.py` / `undo_apply.py` / `undo_db.py` / `undo_world.py` + `undo_archive.py` verified archive commands + `undo_timeline.py` timeline commit + `undo_support.py`) + `world_events.py` (the world's clock: wait for it, announce it live) + the `bot_*` family (the AI window's use cases, the Grok/provider transport, prompt templates + their presets, reaction labels, named connections, transcript shaping) |
...
| UI | `ui/` (29 JS) | Grid, stack DnD, archive windows, collector panel, labels, db panel, composer, log, the two AI windows (`bot-*.js`, `dark-select.js`) |
