...
| Orchestration | `services/` (36) | `run/` (engine), `history/` (service + `trash.py` session-sized trash + `migrate.py` install migration), collector, db lifecycle + deletion, layout, people, undo + `undo_archive.py` (verified archive commands) + `undo_timeline.py` (timeline commit) + `world_events.py` (the world's clock: wait for it, announce it live) |
