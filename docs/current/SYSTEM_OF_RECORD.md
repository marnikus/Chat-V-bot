...
| **Message archive** | Append-only per-person history, FTS5 search (LIKE fallback), paging that stays stable while collection appends, media downloaded and filed per person; every writer of the world file serializes on one gate | `stores/history_*`, `stores/world_lock.py`, `services/history/`, `backend/history_query.py` (paging / stats / the person list) + `backend/history_query_search.py` (the FTS5-or-LIKE back-end and the request-text-to-SQL guards) | `tests/test_history_*`, `tests/unit/stores/`, `tests/integration/services/test_history_service_contract.py` |
...
| Blocks | `actions/` (26) | The 17 action blocks + `BaseAction`, registry, cancellation, wait-speed scaling |
| Page-facing | `backend/` (58) | CDP client family (`cdp_client.py` facade + `cdp_client_transport.py` wire + `cdp_client_events.py` event fan-out + `cdp_client_wire.py` command verbs), DOM probes, the chat parser family (`chat_parser.py` public half + `chat_parser_gate.py` decision machinery + `chat_parser_settle.py` wait policy), chat sync, scroll parser, visual click, the config family (`config_manager.py` facade + `config_defaults` · `config_owners` · `config_view`), the history-query family (`history_query.py` facade + `history_query_rows.py` projection + `history_query_reads.py` SQL), the media family (`media_handler.py` policy + `media_dialog.py` page side + `media_handler_js.py` payloads), the DOM-highlight payload module (`dom_highlight_js.py`); **compatibility shims** for the pre-split names |
| Wire | `bridge/` (25) | `bridge/router.py` — ONE QObject on the QWebChannel, assembled from eleven domain bridges: cdp · stack · people · history · label · db · collector · undo · layout · file · window-preset. The archive bridge is a wire facade only (`history_bridge.py`: the seven signals + twenty-one @Slots) over the five `history_bridge_*` part modules (the shared `wire` guard/scheduling policy · reads · deletions · media+clipboard · settings), and the router itself is three files: the assembly (`router_assembly.py`, the Shiboken-synthesised class), the compat surface (`router_legacy.py`) and the boot path + live slots (`router.py`). Its constructor takes a `BridgeContext` (`Router(ctx=…)`), the historical boot keywords being built into one |
| Orchestration | `services/` (87) | `run/` (engine), `history/` (service + `trash.py` session-sized trash + `migrate.py` install migration), collector (the `collector_*` family), db lifecycle + deletion (the `db_deletion_*` family), layout, people, undo (the `undo_*` family: `undo_service.py` facade + `undo_history.py` / `undo_apply.py` / `undo_db.py` / `undo_world.py` + `undo_archive.py` verified archive commands + `undo_timeline.py` timeline commit + `undo_support.py`) + `world_events.py` (the world's clock: wait for it, announce it live) |
| Persistence | `stores/` (45 files, 15 modules) | SQLite world store + schema/repair, JSON stores, labels, media, presets, undo, `world_lock.py` (one write gate per world file) |
| Shell | `app/` (4) + `main.py` | Bootstrap/DI, window, lifecycle |
| UI | `ui/` (29 JS) | Grid, stack DnD, archive windows, collector panel, labels, db panel, composer, log |
