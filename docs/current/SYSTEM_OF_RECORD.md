...
| **Scroll & Parse** | Harvests the CDK virtual-scroll list, reports each person as found, applies the block's own filter selects, and (opt-in) purges rejects from the queue | `backend/scroll_parser.py`, `actions/scroll_parse.py` | `tests/test_scroll_parse_pipeline.py`, `tests/test_scroll_only_seek.py`, `tests/test_filter_purge.py` |
...
| **I-5** | Only entities that pass the *current* filter are ever persisted. Deleting stored records that now fail it is opt-in (`purge_rejected`, default off); with it on, a stricter re-run shrinks the list | `on_reject` + purge (RULE 6) |
