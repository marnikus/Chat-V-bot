...
| **Grok prompt editor** | The prompt templates are editable in their **own window** (opened by the `[edit]` button beside each action), with a live preview, Save/Cancel and Reset; edits persist between sessions in the `grok` config section | `services/bot_prompts.py`, `bridge/bot_bridge.py`, `ui/js/bot-prompt.js` | `tests/unit/services/test_bot_chat_service.py`, `tests/test_bot_bridge.py` |
