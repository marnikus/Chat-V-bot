...
| [`2026-09-13-round-f/`](#2026-09-13-round-f) | 1 | Round F's closing steps: the wide-parameter tail and `stores/` measured at 15 modules. |
| [`2026-09-13-round-g/`](#2026-09-13-round-g) | 3 | Round G: unfreezing AREA D. The snapshot learns to walk into packages, which is what made every later round's `backend/` splits legal — plus the G1/G3 splits themselves and the round plan. |
...
- [`F5_PARAMETER_OBJECTS.md`](2026-09-13-round-f/F5_PARAMETER_OBJECTS.md) — Round F's wide-parameter tail: which long argument lists became parameter objects, which were left alone because the arguments are genuinely independent, and the `stores/` module count measured at 15 (the number `tools/metrics/stores_modules.py` now ratchets).

## 2026-09-13-round-g

- [`ROUND_G_PLAN_2026-09-13.md`](2026-09-13-round-g/ROUND_G_PLAN_2026-09-13.md) — The round plan: what the 500-line tail still held after Round F, and the order the steps had to run in.
- [`AREA_D_DECISION_2026-09-13.md`](2026-09-13-round-g/AREA_D_DECISION_2026-09-13.md) — The decision that unblocked every later round. The AREA D public-API snapshot could not see into packages, which froze `backend/` against exactly the remedy its biggest files needed; step G0 taught it to walk into packages and to count a symbol defined in a submodule as owned by its package. Proven by a byte-identical dump of the unchanged tree, and by three splits that pass `test_backend_api_snapshot.py` **unrefreshed**. The two constraints it attaches — re-export the previous surface verbatim, and LCOM beats line count when they disagree — are still in force and still quoted in `AGENT_RULES.md` §18.2.
- [`ROUND_G_G1_G3_SPLITS_2026-09-13.md`](2026-09-13-round-g/ROUND_G_G1_G3_SPLITS_2026-09-13.md) — The splits themselves: `chat_sync` 807, `scroll_parser` 706, `history_query` 603, and why G3 left `HistoryQuery` (LCOM 0.74) whole and split the module around it instead.

## 2026-09-13-round-h

- [`ROUND_H_PLAN_2026-09-13.md`](2026-09-13-round-h/ROUND_H_PLAN_2026-09-13.md) — The god-class round: ten files, each split on the semantic boundary it already had rather than by line count, and the three seam shapes the round established — (a) two-job file, (b) cohesive class whose names are an external contract (mixins, class identity kept for Qt), (c) genuine artefact. **Caveat:** the closing report's per-step size table was later found to state the plan's targets rather than the measured results; `reports/ROUND_H_CLOSING_2026-09-13.md` now carries both columns and the correction.
