...
| [`2026-09-13-round-g-write-gate/`](#2026-09-13-round-g-write-gate) | 6 | Round G: the complete post-Round-F tail inventory with fresh measurements, the prioritised G1–G7 step plan, and the executed steps G1–G6 — the red suite at HEAD and the `WriteTurn` union fix (F3c), the two worst-file family splits under the lifted freezes, the flow/injector splits plus the ladder and constructor reductions, and the wide-parameter continuation that took the >4-param walker from 51 to its 18-entry floor, the test-debt step that took the three undo modules to 100% and paid the F6b module-wide mutation run, and the hygiene step that zeroed the tree's cognitive-17 offenders and paid the rules file back inside its budget. |
...
own doc below. Steps G5–G7 stay planned, sized ≈ 8–16 h each.
...
## 2026-09-14-ai-bot-chat-port

Reapplying the finished AI Bot Chat + Grok Prompt Editor feature (the group above) on this tree after
Rounds F and G moved the code underneath it: the feature's own files come over verbatim where they could,
and the six files the refactor had moved were resolved one by one, with the exit-hang the ported database
tests exposed fixed in the tests rather than papered over.

*1 doc.*

- [`PORT_DESIGN_2026-09-14.md`](2026-09-14-ai-bot-chat-port/PORT_DESIGN_2026-09-14.md) — The port record (RULE 16 §16.6.2): the feature branch `arena/01a09b6e-chat-v-bot` @ `478fca5` is the source (the branch the task named, `arena/01a09b51-chat-v-bot`, holds none of it; the parallel `arena/01a09cb8-chat-v-bot` port is two fix commits short of it — it lacks the popup CSS scoping and the pinned close cross), `0ad07c9` is the common ancestor, and of the 24 ported files the feature modifies 18 were byte-identical to this tree and applied verbatim (`layout_bridge`, `layout_service`, `preset_store`, `ui/index.html`, five `ui/js` modules, nine tests, `SYSTEM_OF_RECORD.md`). The six that had moved are resolved in place: `bridge/router.py`'s edits re-applied into G6's `_register_*` phases instead of re-inlining `_build_router_class`, `backend/config_manager.py`'s four routing lines, `tools/metrics/rule16_gate.py` merged table by table — OWNED verbatim, `SMELL_FILES` **keeping** `history_query.py`/`history_bridge.py` (which H2 no longer has) plus the six bot files, and `bot_bridge.py` joining the cdp/people clone-header group, proven by `--with-clones` reporting 0 new and 0 stale. §4 is the refactor the port had to speak (`HistoryService(cdp=None)` → `HistoryService(HistoryDeps())` in the test that builds the REAL archive), §5 the bug the port found — five classes of `tests/unit/services/test_bot_chat_service.py` opened real worlds and never closed them, and aiosqlite's non-daemon worker thread parked the interpreter in `threading._shutdown` after a fully green 3 030-test run (`py-spy` in one shot; rc 124 at 10 minutes), fixed by closing every world in `WorldCase.tearDown` — and §7 the numbers: 3 030 passed / 898 subtests, 28 JS suites green, coverage 91.59 % line / 87.76 % branch, 143 new functions with 0 over 30 LOC and none uncovered, RULE 16 gate clean. §9 also checks that the ported tests bite: one function per new module renamed in a throwaway copy of the tree, and all 16 deletions noticed (13 Python, 3 JS). §8 records the RULE 18 recheck: three AI JS controllers (396/394/354 lines) over the 300 ideal with their `ideal-size:` reasons kept and three stale numbers in those notes corrected to the measured physical count, stylesheets noted as outside the file ideal by house practice, and `SYSTEM_OF_RECORD.md` 328 → 341 lines (13 new one-line rows) on a file the rules already call "at its ceiling". §6 records that no `config/*.json` is copied at all — the saved v4 tree (a v3 payload is upgraded, not rejected), the undo timeline, the presets with the owner's key, the settings — and that the feature seeds what it needs itself: one keyless row per provider, refused by `client_for` until a key exists.
