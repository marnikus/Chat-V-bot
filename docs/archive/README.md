...
| [`2026-09-13-round-g-write-gate/`](#2026-09-13-round-g-write-gate) | 5 | Round G: the complete post-Round-F tail inventory with fresh measurements, the prioritised G1–G7 step plan, and the executed steps G1–G5 — the red suite at HEAD and the `WriteTurn` union fix (F3c), the two worst-file family splits under the lifted freezes, the flow/injector splits plus the ladder and constructor reductions, and the wide-parameter continuation that took the >4-param walker from 51 to its 18-entry floor, and the test-debt step that took the three undo modules to 100% and paid the F6b module-wide mutation run. |
