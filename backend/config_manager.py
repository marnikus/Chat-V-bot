...
# Round H (H4) RETIRED the `# ideal-size: 502` exemption that stood here. It
# claimed the AREA D public-API snapshot made this file unsplittable. That was
# true only of the SHAPES it considered (a package, or a re-export shim), and
# it was re-derived rather than quoted: dump_public_api.py skips every name
# starting with `_`, so the five section-owner classes — all private, and none
# of which reaches back into ConfigManager — were never part of the contract
# at all. They moved to backend/config_owners.py and the snapshot did not
# move: 509 -> 181 lines with the public API byte-identical.
...
from backend.config_owners import (_UNSET, _deep_merge, _DictOwner, _ListOwner, _NamedOwner,
                                   _Owner, _SettingsOwner)
