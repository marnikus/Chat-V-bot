"""Paging limits, the sortable-column whitelist, and the tiebreak.

Owns the constants the read path is configured by. `SORT_COLUMNS` IS the
whitelist: a `sort` key that is not in it falls back to `DEFAULT_SORT`, so no
request text can ever reach `ORDER BY`.
"""
