"""Archive write policy and live-result aggregation for a sync session.

Imports point to store value types only, never to the runtime session, facade,
Qt or services. A caller supplies the shared session; repository writes remain
ordered by that caller. Cancellation is not a best-effort recovery failure.
"""
...
# ideal-size: class exceeds 120 lines reason=the frozen 12-method archive
# write interface shares one session/repository policy; retain its signatures.
class SyncPersister:
    ...
    # ideal-size: 28 lines reason=the tail probe/read/recover sequence shares
    # one best-effort exception boundary; cancellation must propagate through it.
