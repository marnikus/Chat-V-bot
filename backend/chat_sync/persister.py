"""`SyncPersister` — every write to the archive, in one place.
...
References `SyncSession` only in string type hints (`"SyncSession"`) so it
never imports `session` back — the session *owns* its persister.
"""
