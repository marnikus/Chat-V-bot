"""`SyncOptions` — the eleven optional knobs of one sync, immutable.

Owns the *configuration* of a sync and the small pieces of behaviour the
phases ask of it (pause timing, the stop predicate, progress reporting).
Imports nothing from the rest of the package: it is a leaf in the dependency
order, imported by `plan`, `session` and `sync`.
"""
