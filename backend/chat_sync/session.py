"""`SyncSession` — the state one sync shares between its phases.

The mutable heart of a sync: the handful of values the phases agree on and
that may move while a read is in flight (`count`, the signatures, the cursor).
It owns the opening phases (probe → gate → viewport → archive), the closing
phases (empty / cap-gap / restore / finish) and the `absorb` fold that turns
each `AppendResult` into the `SyncResult` the caller sees.

Imports down the dependency order only: `options`, `plan`, `persister`,
`merge`, and `stores.history_models`. The `reader` and `aligner` import it
back (via string type hints), never the reverse.
"""
