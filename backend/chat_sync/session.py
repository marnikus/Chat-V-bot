"""The state one sync shares between its phases, and the phases that move it.

Owns: `SyncSession` — the opening phases (probe, RULE 15 gate, viewport,
cursor), the mutable state the phases agree on, and the closing phases (cap
gap, viewport restore, final cursor, totals, reason).

Why the state is an object and not a pile of locals: the page can change
*during* a read — a virtualised pane that loses its nodes mid-read updates the
count and the signatures — and the final cursor write must reflect what was
actually read, not what we hoped for.
"""
...
class SyncSession:
    ...
    async def _pass_gate(self) -> bool:
        ...
        Round H (step H5) moved the gate out of `backend.chat_parser` into
        `backend.private_gate`, which imports nothing from chat_sync — so the
        load-time cycle that forced this import to be function-local is gone.
        It stays local only because `norm` beside it is, and hoisting both is
        a separate change.
        """
