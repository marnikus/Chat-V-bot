"""`DeltaAligner` — where a freshly read conversation continues the stored one.

Owns the overlap detection between a buffered re-read and the archive's tail.
Imports `stores.history_repo.align_batch` and references `SyncSession` only in
