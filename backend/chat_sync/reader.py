"""`ChunkReader` — read `[start, count)` in paced, retrying chunks.

Owns the two nested loops (chunk loop × slice retries) so the sync body does
not. Each chunk goes straight to the :class:`SyncPersister` when the read is
streaming, or into `session.collected` when the archive has a tail we must
align against first.

Imports `constants` (SLICE_RETRIES) and references `SyncSession` only in
string type hints; it never imports `session` back.
"""
...
from .constants import SLICE_RETRIES
