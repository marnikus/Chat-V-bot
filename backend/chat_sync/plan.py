"""The sync decision, as a pure function — `ReadPlan` + `SyncPlanner`.

Owns the delta/empty/unchanged/full decision and the window arithmetic. It is
deliberately page-free: `plan()` reads a state dict and a cursor dict and
returns a :class:`ReadPlan`, which is what makes it unit-testable without a
DOM (tests/unit/backend/test_chat_sync_plan.py).

Imports: `constants` (modes), `options` (SyncOptions), `backend.chat_text`
(the signature helper). It never imports `session`, `persister`, `reader` or
`aligner` — those all depend on *its* decision.
"""
