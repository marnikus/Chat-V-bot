...
Owns: `SyncPersister`. `HistoryRepo` is the only thing that may touch the
database, so the sync never calls it from two different layers — cursor
bookkeeping, gap notes, record appends, the backfilled flag and media
recovery all funnel through here.

Imports the session only as a type at runtime-free scope (the annotation is a
string), so `session` may import this module back without a cycle.
"""
