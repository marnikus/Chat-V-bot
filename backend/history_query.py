...
def _sort_spec(sort: str) -> tuple[tuple[str, str], ...]:
    """The whitelisted column spec for a requested sort key."""
    return SORT_COLUMNS.get(str(sort or ""), SORT_COLUMNS[DEFAULT_SORT])


def _resolved_dir(sort: str, direction: str) -> str:
    """`"asc"` / `"desc"` — the direction actually applied.

    An empty (or unrecognised) `direction` means *the key's natural
    direction*, which is the direction of its first column. Reporting the
    resolved value back is what lets the header show the right arrow without
    duplicating the table above in JavaScript.
    """
    asked = str(direction or "").strip().lower()
    if asked in ("asc", "desc"):
        return asked
    return _sort_spec(sort)[0][1].lower()


def _order_by(sort: str, direction: str) -> str:
    """Build the `ORDER BY` body from the whitelist — never from the request.

    `direction` flips **every** column of the key, so the secondary column
    stays consistent with the primary one (`msgs` descending = busiest first,
    and among equals the *oldest* activity last).
    """
    asked = str(direction or "").strip().lower()
    parts: list[str] = []
    used: set[str] = set()
    for column, natural in _sort_spec(sort):
        ...
        if asked in ("asc", "desc"):
            ...
            want = natural
        parts.append(f"{column} {want}")
    for column in SORT_TIEBREAK:
        if column in used:
            continue
        used.add(column)
        parts.append(f"{column} ASC")
    return ", ".join(parts)
...
class HistoryQuery:
    ...
    async def list_persons(self, q: str = "", limit: int = DEFAULT_LIMIT,
                           ...
                           dir: str = "", include_deleted: bool = False) -> dict:
        limit = self._clamp(limit)
        ...
        order = _order_by(sort, dir)
        if needle:
            # Relevance first: an exact prefix of the needle outranks the
            # chosen column order, but the column order still decides inside
            # each tier.
