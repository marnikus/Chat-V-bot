...
class HistoryBridge(QObject):
    ...
    def attach_history(self, service):
        ...
        if service and hasattr(service, "collector"):
            try:
                service.collector.history_appended.connect(lambda p: self.userdb_changed.emit(p))
            except Exception:
                pass
    ...
    async def _page(self, req_id, nick, opts):
        ...
        if opts.get("around") is not None:
            ...
            payload["stats"]=await svc.query.person_stats(nick)
            payload["my_nick"]=svc.my_nick
        else:
