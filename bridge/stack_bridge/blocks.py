...
`CustomBlockMixin` owns the reusable block configs in the block store:
the list the + Add menu and the chips read, one save that validates
through the store's `Result` (a refusal says why), and a delete that
only announces a removal it actually made.
"""
