...
CLONE_BASELINE = frozenset({
    ...
    # AI Bot Chat (2026-09-13) joined the existing cdp/people group: the
    # same standard bridge header, no logic copied. Every name is used —
    # `asyncio.ensure_future` schedules the Grok call, `json.dumps` shapes
    # every answer, `log.warning` reports a raising service, the three Qt
    # names declare the slots and signals, `LogMessage` puts a failure in
    # the log console. See the maintenance note below.
    ("bridge/bot_bridge.py", "bridge/cdp_bridge.py",
     "bridge/people_bridge.py"),
    # The standard bridge import headers — a match of imports, not logic.
    # Shrinking one would mean deleting a used import or reordering into a
    # pylint C0411 warning: the cosmetic change §18.5 forbids.
    # (`bot_prompt_bridge.py` was in this group until 2026-09-13, when the
    # Prompt Editor and the AI Settings dialog moved their shared wiring
    # into `BotSideBridge` and its import list changed.)
