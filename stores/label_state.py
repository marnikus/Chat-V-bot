...
def _clean_def(item: dict) -> dict | None:
    """One def row, normalised; None when it cannot be a label at all."""
...
def _clean_defs(raw: dict) -> tuple[list[dict], set[str]]:
    """Normalised defs in first-seen order + the set of surviving ids.

    An id or a case-folded name seen twice keeps its first definition —
    the later row is the duplicate, never the replacement.
    """
...
def _clean_assign(raw: dict, seen_ids: set[str]) -> dict[str, list[str]]:
    """Nick → label ids it actually holds: known ids only, deduped in order."""
...
def _known_ids(values, seen_ids: set[str]) -> list[str]:
    """A stored id list reduced to known ids, first occurrence kept."""
    kept = [str(i) for i in (values or [])
            if str(i) in seen_ids]
...
def _clean_filter(raw: dict, seen_ids: set[str]) -> dict:
    ...
    include = _known_ids(raw_filter.get("include"), seen_ids)
    ...
    # stale include tick.
    exclude = _known_ids(raw_filter.get("exclude"), seen_ids)
